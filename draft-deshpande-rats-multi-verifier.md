...
The Verifier needs to ensure that the claims included in the Evidence reflect the lasteste state of the Attester. The Synchronized Clocks, Epoch IDs, and nonce
can serve this purpose when they are embeded inside the Evidence. The utilization of the Synchronized Clocks and Epoch IDs is the same as the single Verifier
case specified by the RFC9334. For the nonce, in the Lead Verifier case, it is suggested that the nonce is uniquely generated by the Lead Verifier, and the
attester and sub-Verifiers receive the nonce from the Lead Verifier; in the Cascaded Pattern, the nonce for the generation of the evidence is generated by the
first Verifier that the attester contacts. 
