...
The Verifier needs to ensure that the claims included in the Evidence reflect the lasteste state of the Attester. Some of the  examples, include synchronized clocks, Epoch IDs, and nonce. The utilization of the Synchronized Clocks and Epoch IDs in the multiple Verifiers case is the same as the single Verifier
case specified by the RFC9334. When nonce is used in the Lead Verifier case, it is suggested that the nonce is uniquely generated by the Lead Verifier, and the attester and sub-Verifiers utilize the nonce from the Lead Verifier. When nonce is used in the Cascaded Pattern, it is suggested that the nonce is generated by the
